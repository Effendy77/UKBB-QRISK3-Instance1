...
for code_col in icd10_code_cols:
    ...
    mask = (
        icd[code_col].notna() &
        icd[code_col].astype(str).str.startswith(("N18","N19")) &
        (icd[date_col] <= icd["53-1.0_dt"])
    )

    icd.loc[mask, "ckd_inst1"] = 1
...
for code_col in icd9_code_cols:
    ...
    mask = (
        icd[code_col].notna() &
        icd[code_col].astype(str).str.startswith(("585","586")) &
        (icd[date_col] <= icd["53-1.0_dt"])
    )

    icd.loc[mask, "ckd_inst1"] = 1

print("\nCKD prevalence at Instance 1:")
print(icd["ckd_inst1"].value_counts())
print("Prevalence:", icd["ckd_inst1"].mean())
