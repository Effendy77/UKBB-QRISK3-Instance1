...
for code_col in icd10_code_cols:
    ...
    mask = (
        icd[code_col].notna() &
        icd[code_col].astype(str).str.startswith(("M05","M06")) &
        (icd[date_col] <= icd["53-1.0_dt"])
    )

    icd.loc[mask, "ra_inst1"] = 1
...
for code_col in icd9_code_cols:
    ...
    mask = (
        icd[code_col].notna() &
        icd[code_col].astype(str).str.startswith("714") &
        (icd[date_col] <= icd["53-1.0_dt"])
    )

    icd.loc[mask, "ra_inst1"] = 1

print("\nRA prevalence at Instance 1:")
print(icd["ra_inst1"].value_counts())
print("Prevalence:", icd["ra_inst1"].mean())
