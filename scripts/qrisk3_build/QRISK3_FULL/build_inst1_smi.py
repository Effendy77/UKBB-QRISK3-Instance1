...
print("Computing SMI_inst1...")

icd = icd.copy()
icd["smi_inst1"] = 0

# ICD10: F20, F25, F30, F31
for code_col in icd10_code_cols:
    date_col = code_col.replace("41270","41280")

    mask = (
        icd[code_col].notna() &
        icd[code_col].astype(str).str.startswith(("F20","F25","F30","F31")) &
        (icd[date_col] <= icd["53-1.0_dt"])
    )

    icd.loc[mask, "smi_inst1"] = 1
...
for code_col in icd9_code_cols:
    ...
    mask = (
        icd[code_col].notna() &
        icd[code_col].astype(str).str.startswith(("295","296")) &
        (icd[date_col] <= icd["53-1.0_dt"])
    )

    icd.loc[mask, "smi_inst1"] = 1

print("\nSMI prevalence at Instance 1:")
print(icd["smi_inst1"].value_counts())
print("Prevalence:", icd["smi_inst1"].mean())
